...
description: Preprints and working papers, with supplementary proofs and future directions.
nav: false
nav_order: 4
---
...
{% for p in site.data.in_progress.preprints %}
  <li>
    ...
    {% if p.supplementary %}<div class="pub-supp">+ Supplementary proofs: {% for s in p.supplementary %}{{ s.label }} <a href="{{ s.url }}">PDF</a>{% unless forloop.last %} &middot; {% endunless %}{% endfor %}</div>{% endif %}
  </li>
{% endfor %}
</ul>
